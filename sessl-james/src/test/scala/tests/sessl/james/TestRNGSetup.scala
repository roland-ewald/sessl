...
class TestRNGSetup {
  ...
  @Test def testRNGSetup() = {
    ...
    def testFunction(rngClassName: String)(x: AnyRef): Unit = {
      ...
      assertEquals("The correct seed shall be used as the initial seed.", testSeed, SimSystem.getRNGGenerator.asInstanceOf[CurrentTimeRandSeedGenerator].getSeedGeneratorSeed)
    }
    ...
    val exp1 = new Experiment {
      ...
      stopTime = .01
      rng = LCG(testSeed)
      afterExperiment(testFunction(classOf[org.jamesii.core.math.random.generators.lcg.LCG].getName))
    }
    ...
    val exp2 = new Experiment {
      ...
      afterExperiment(testFunction(classOf[org.jamesii.core.math.random.generators.mersennetwister.MersenneTwister].getName))
    }
    execute(exp1, exp2)
  }
}
