...
@Test class ParamBlockGeneratorTest extends Logging {
  ...
  @Test def testAlgorithmSetConversion() {
    val tlSetups = AlgorithmSet(TauLeaping(gamma = 11) scan { "epsilon" <~ range(0.01, 0.01, 0.03) })
    ...
    testFactoryClassSetup(TauLeaping().factory.asInstanceOf[Factory], tlParamBlocks)

    val nrSetups = AlgorithmSet(NextReactionMethod() scan { "eventQueue" <~ (SortedList(), LinkedList()) })
    ...
    testFactoryClassSetup(NextReactionMethod().factory.asInstanceOf[Factory], nrParamBlocks)
