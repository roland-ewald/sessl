...
class TestBiBJamesExperiment {
  ...
  @Test def testBriefingsExperiment() = {
    ...
    val exp = new Experiment with Observation with ParallelExecution with DataSink with Optimization {
      ...
      optStopCondition = OptMaxAssignments(10) or OptMaxTime(hours = 1) //diff: was 100, but this takes too long for testing, no time constraint! ;-)
